...
if [[ $vps = "vps" ]]; then
	source="https://raw.githubusercontent.com/reyluar18/Shugar_OCS/master"
else
	source="https://raw.githubusercontent.com/reyluar18/Shugar_OCS/master"
fi
...
read -p "Password: " -e -i Shugar DatabasePass
echo ""
...
read -p " Database Name: " -e -i OCS_PANEL_Shugar DatabaseName
echo ""
...
echo "Installing Shugar VPN Theme"
cd ~
mkdir Rey
cd Rey
rm -rf *
#change mo yung link sa paguuploadan mo ng dropbox
wget https://www.dropbox.com/s/0828vemag3d6t3y/ShugarVPN.tgz && tar zxvf ShugrVPN.tgz 
...
echo "" | tee -a log-install.txt
echo "Auto Script Installer OCS Panels Mod by shugar"  | tee -a log-install.txt
echo "                        "  | tee -a log-install.txt
echo "" | tee -a log-install.txt
